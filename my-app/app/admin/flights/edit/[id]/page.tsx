...
function EditFlight() {
    ...
    useEffect(() => {
        ...
        const getFlights = async () => {
            try {
                ...
                setDepartureTime(formatDateTimeLocal(res.data.departureTime));
                setArrivalTime(formatDateTimeLocal(res.data.arrivalTime));
                setRoute(res.data.route);
                setStatus(res.data.status);
                setPrice(res.data.price);
                setAircraftId(res.data.aircraftId || "");
            } catch (err) {
                router.push('/login');
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = { flightNumber, departureTime, arrivalTime, route, status, price: Number(price) };
        ...
        try {
            ...
            if (resp.status === 200) {
                if (errorElement) {
                    errorElement.textContent = "Flight updated successfully!";
                    errorElement.style.color = "green";
                }
            }
            else {
                if (errorElement) {
                    errorElement.textContent = "Failed to update flight.";
                    errorElement.style.color = "red";
                }
            }
        } catch (err: any) {
            ...
            if (submitBtn) {
                ...
                submitBtn.innerHTML = `Update Flight`;
            }
            setSubmitting(false);
        }
    };

    {
        return (
            <>
                ...
                (
                <div className="min-h-screen bg-base-100">
                    <h1 className="text-4xl font-bold text-center pt-10 text-base-content">Edit Flight</h1>
